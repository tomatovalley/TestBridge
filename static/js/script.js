...
$(document).ready(function(){
    ...
    $(".card-header-right .reload-card").on('click',function(){
        ...
        setTimeout(function(){
            $this.parents('.card').children(".card-loader").remove();$this.parents('.card').removeClass("card-load");
        },3000);
    });
    $(".card-header-right .card-option .open-card-option").on('click',function(){
        var $this=$(this);if($this.hasClass('icon-x')){
            ...
            $this.parents('.card-option').children(".first-opt").fadeIn();}else{$this.parents('.card-option').animate({'width':'130px',});
    ...
    $(".card-header-right .full-card").on('click',function(){
            ...
            $(this).toggleClass("icon-minimize");$(this).toggleClass("icon-maximize");
    });
    ...
    $(".search-close").on('click',function(){
        ...
        setTimeout(function(){
            $(".main-search").removeClass('open');},300);
    });
    /*$("#styleSelector .style-cont").slimScroll({
        setTop:"1px",height:"calc(100vh - 480px)",
    });
    var a=$(window).height()-80;
    $(".main-friend-list").slimScroll({
            height:a,allowPageScroll:false,wheelStep:5
    });
    ...
    $('.displayChatbox').on('click',function(){
        ...
        if(my_val=='right'){var options={direction:'left'};}
        else{var options={direction:'right'};}
    ...
    $('.form-control').on('blur',function(){
        if($(this).val().length>0){$(this).addClass("fill");}
        else{$(this).removeClass("fill");}
    });
    $('.form-control').on('focus',function(){
        $(this).addClass("fill");
    });
    /*la funcion enta en pcoded */
...
$(document).ready(function(){
    ...
    $('#toggleSideBar i').addClass('icon-toggle-right');
    $('#toggleSideBar').on('click', function () {
      $('#toggleSideBar i').toggleClass('icon-toggle-right');
      $('#toggleSideBar i').toggleClass('icon-toggle-left');
    });
